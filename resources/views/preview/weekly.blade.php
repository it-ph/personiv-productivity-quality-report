...
@section('content')
	...
	<div>
	  	...
	  	<ul class="nav nav-tabs projectTabs" role="tablist">
		    @foreach($projects as $project)
		    	@if(count($project->reports))
			    	<li role="presentation"><a href="#{{$project->id}}" aria-controls="{{$project->id}}" role="tab" data-toggle="tab">{{$project->name}}</a></li>
			    @endif
		    @endforeach
	  	</ul>
	  	...
	  	<div class="tab-content">
	  		@foreach($projects as $project)
	  			@if(count($project->reports))
			    	<div role="tabpanel" class="tab-pane" id="{{$project->id}}">
			    		<table class="table">
							<tr>
	...
	<script>
		$('document').ready(function(){
			$('.projectTabs li:first, .tab-content .tab-pane:first').addClass('active');
		});
	</script>
@stop